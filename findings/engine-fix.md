...
## IPC filenames hole (historical — fixed)

Was: `SelectionData.serialization.in` omitted `filenames`, so UIProcess grants died on the hop.

Now fixed in `17647b75df`: `Vector<String> filenames()` on the wire; decode via `setFilenames` after `setURIList` with no re-promotion. See `findings/opus-second-opinion.md` scorecard.

## Build fix: WTFMove include

`SelectionData.cpp` needs `#include <wtf/StdLibExtras.h>` so `WTFMove` in `setFilenames` / URI list helpers compiles under the unified sources build. Landed on the fork after unit run 31242490849 failed on that error. Later tip prefers `WTF::move`.

## Engine tip notes

- `a7afbb6618` … `a85db85942` — original four-layer trust split stack
- `17647b75df` — H1 serialize `filenames` over IPC; IPC constructor unit
- `2b70a3d087` — clipboard write `uriListWithoutFilenames` + GTK4 `m_pendingDrop`
- Product proof still needs nested/AppImage E2E (S1/F1), not just units

## Follow-ons: H2, H3, path canon, H7 (not E2E blockers)

Order: **prove E2E on current tip first**. These are additive commits or a design fork — do not rewrite the landed stack.

### H2 — narrow IsSource (product policy)

**Problem:** GTK sets `DragApplicationFlags::IsSource` for any in-process drag:

- GTK4 `DropTargetGtk4.cpp` `applicationFlagsForDrop`: `gdk_drop_get_drag(drop)` non-null
- GTK3 `DropTargetGtk3.cpp`: any `gtk_drag_get_source_widget(context)`

Cocoa sets IsSource only when the source is **this** web view. `DragDataGLib.cpp` `allowsFilenameAccess` denies all filenames when IsSource is set. So same-app **native file widget → WebView** over-denies (product gap). Web→Web and Nautilus→Web are fine for Epiphany F1.

**E2E check (no code change):**

1. Nautilus → page: files nonempty (external; IsSource false) — F1
2. Page drag file:// uri-list → self: files empty — S1
3. Optional harness: Gtk list/GdkFileList in same process as WebView → expect DENY today; document. Only matters for embedders.

**Fix commit (optional, own commit after E2E):** narrow flag to this `m_webView` / this DragSource's `GdkDrag`. Keep export sanitize. Do not fold into a85db.

### H3 — trusted filenames API shape

**Problem:** `SelectionData::setFilenames` / `setFilenamesFromURIList` are public. Policy is comment-only ("UIProcess DropTarget only").

**Where:** `SelectionData.h/.cpp`; callers DropTargetGtk3/4 + IPC constructor.

**Options:** rename to `setTrustedFilenames`; or private + friend DropTarget + internal apply for IPC decode. Runtime unchanged if callers already correct.

**Commit:** small own commit if reviewers want API hygiene. Skip before E2E. Not a fold into layer 1.

### Path canonicalization (residual, pre-existing)

**Today:** `g_filename_from_uri` only in `filenamesFromURIList` and DropTarget portal branch. No `realpath`, no mandatory native-only filter on drop grants. `file://127.0.0.1/...`, symlinks, odd forms behave like classic GTK file DnD.

**Where if ever:** centralize in `SelectionData::filenamesFromURIList` + portal path in `DropTargetGtk4.cpp`. Optional `g_file_is_native` (clipboard GTK4 already skips some non-native).

**Commit:** separate optional hardening with explicit tests and reviewer buy-in. Document "no symlink resolution" on the bug. Not part of CVE restore. Do not claim this series solves it.

### H7 — parallel `allowedFiles` on drag IPC (alternative design)

**Idea:** SelectionData stays string-only; `PerformDragControllerAction` carries `Vector<String> allowedFiles` beside selection. Web process grants only from that vector.

**Where (large):** `WebPage.messages.in` / generated IPC; `WebPageProxy.cpp` GTK/WPE send path; `WebPage::performDragControllerAction`; possibly DragData; remove or ignore filenames on `SelectionData.serialization.in`; WPE same path; all tests.

**vs H1 (shipped):** filenames on SelectionData is smaller and works with existing coder.

**Commit strategy:** not a stack-on-top polish. Implementing H7 **supersedes** `17647b75df`. Only if upstream rejects filenames-in-SelectionData. Do not build H7 now.

### Stack picture

```
landed: trust split four + IPC filenames + clipboard/async
---- E2E proof (AppImage + nested S1/F1) ----
optional own: H2 IsSource narrow
optional own: H3 API tighten
optional later: path policy
fork replace: H7 only if design rejected
```

Upstream squash may collapse security stack to 1–3 reviewable commits. Private fork keeps granularity. Never rewrite a7afbb… history to insert H2/H3.

## Layer 5: the file URI hostname hole (found by our own tests)

This is a real vulnerability that our patch shipped until the rewritten
negative control caught it. It was not in the original CVE and it is not in the
upstream workaround. We introduced the exposure by re-enabling a grant path and
then trusting glib to validate the URI for us.

### What glib actually does

`g_filename_from_uri()` has this signature:

```
gchar *g_filename_from_uri (const gchar *uri, gchar **hostname, GError **error)
```

If you pass `NULL` for `hostname`, glib parses the authority, confirms it is
syntactically well formed, and then discards it. It does not error. It returns
the path.

Verified empirically against glib 2.86.5 with a standalone C probe:

- `file:///etc/passwd` gives `/etc/passwd`, no error
- `file://localhost/etc/passwd` gives `/etc/passwd`, no error
- `file://attacker.example/home/user/.ssh/id_rsa` gives
  `/home/user/.ssh/id_rsa`, **no error**
- `file://192.168.1.5/etc/passwd` errors with "invalid hostname"

The dotted-quad case erroring is a trap. It makes the hole look closed if that
is the only case you try. DNS-style names sail through.

### Why that mattered here

Our `filenamesFromURIList()` passed `NULL` for the hostname. So a page could
put `file://anything.example/home/user/.ssh/id_rsa` on the drag, and if any
grant path ran over that line, it resolved to the local path and became a
readable `File`. The authority was decoration.

The test that caught it is `remote-file-uri-grants-no-files`. It failed on its
very first run, before the fix existed. That is the value of writing the test
from the threat model rather than from the code.

### The fix, and why import and export are not the same predicate

The first fix attempt used one shared predicate for both directions, on the
theory that a single parser avoids a parser differential. That was wrong, and
the probe is what showed it.

Import and export answer different questions.

`localPathFromURIListLine()` is the **import** side. It decides what the page
is allowed to read. It now takes the hostname out-param and rejects any
non-empty authority that is not `localhost`, compared case-insensitively. If
there is any doubt about a line, it grants nothing. Narrow by construction.

`isFileURIListLine()` is the **export** side. It decides what we hand to
another application when a drag leaves us. It strips anything with a file
scheme, including `file://attacker.example/...`, which import already refused.
The reason is that the receiving application may resolve `file://host/path`
locally, exactly the way WebKit did until this fix. Refusing to grant it
ourselves does not make it safe to emit.

So the invariant is containment, not equality. Everything import grants, export
strips, and export strips strictly more. `SelectionData.EverythingGrantedOnImportIsStrippedOnExport`
asserts that over a table of URIs rather than a single case, so a future edit
that widens import without widening export fails the build.

The general lesson: when a security check has an import side and an export
side, resist the urge to unify them. The asymmetry is the security property.

### Also worth knowing

`"Files"` is not a separate hole.
`selectionDataTypeFromHTMLClipboardType()` maps both `"Files"` and
`"text/uri-list"` to `ClipboardDataTypeURIList`, which reaches the same
non-promoting `setURIList()`. There is no second road in.

## Upstream is building IPC::Untrusted<T>, and a reviewer will ask about it

Noticed while checking whether our base was current. `IPC::Untrusted<T>` landed
upstream in `Source/WebKit/Platform/IPC/Untrusted.h` on 2026-09-01, one day
before the base we are sitting on, under bug 322837. Origin validation
procedures followed the same day under bug 323100. It is being swept through
`WebPageProxy.cpp` now.

What it is, in its own words: a wrapper for a value a web content process sent
to a privileged process, which cannot be used until a declared validation
procedure has confirmed the sender was entitled to name it. Extraction without
validation is possible but has to be spelled
`unsafeExtractWithoutValidation(UnvalidatedReason::...)`, and the reasons are
an enum that includes `NeedsReview` as an explicit bootstrapping state.

That is the same problem our patch solves by hand. `WebPageProxy::startDrag()`
clears filenames on the way up because filenames are a grant the UI process
issues, never something the web process may assert. Expressed in the new
framework, the `SelectionData` arriving from the web process is exactly an
untrusted value and `m_filenames` is exactly the field a validation procedure
would have to vouch for.

So expect the question, from Catanzaro or anyone doing the IPC sweep: why not
wrap it. Answers we can stand behind:

- The framework is days old and the sweep is in progress. Writing a
  `ValidationProcedure` for `SelectionData` means declaring what a legitimate
  web-process-asserted filename list looks like, and the answer is that there
  is no such thing. There is nothing to validate. Clearing is the correct
  operation, not a weaker version of validating.
- Clearing is backport-friendly. This change has to land on stable branches
  where `IPC::Untrusted<T>` does not exist. A version that depends on it cannot
  be backported without dragging the framework along.
- The two are not in conflict. If the sweep later wraps `SelectionData`, the
  clear in `startDrag()` becomes a validation procedure that returns an empty
  filename list, and the security property does not change in the meantime.

Worth saying in the PR discussion rather than waiting to be asked. It reads as
awareness of where the codebase is going instead of ignorance of it. Do not put
it in the commit message; it is reviewer conversation, not a durable
