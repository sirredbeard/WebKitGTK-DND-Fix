...
- `any` → labels `self-hosted,linux,x64,webkit-dnd` (either host)
- pin adds `azure` or `vultr`

Concurrency groups include the runner_label so unit build and AppImage can run at the same time on different machines without canceling each other. AppImage defaults to **azure** (more RAM for full prefix). Unit build defaults to **any**.
