...
## AppImage migrator absolute path (host version mismatch)

Full write-up: `findings/appimage-packaging.md`.

Short version: Epiphany bakes `/usr/libexec/epiphany/ephy-profile-migrator`. Host system epiphany can be the wrong migration version; clean guests lack the path. Same bug class. AppImage docs want same-length `/usr` reloc + cwd=usr, not bwrap binds. We tried bwrap first; wrong. Python same-length rewrite now covers epiphany migrator paths and WebKit `/opt/webkitgtk-dnd` PREFIX leftovers (injected-bundle). `sed -i` on ELF failed closed in CI run 31248832293 with no bytes changed. Smoke must use AppRun only.


## AppImage shell bugs and local lint (not CI)

We keep shipping basic `set -u` / quoting bugs into AppImage CI (comment word bwrap, bare migrator without libs, `"$ORIGIN"` unbound in log line under nounset, single quotes inside smoke `bash -lc '...'`).

Policy: **lint locally before push**, do not add a shellcheck CI job. Use `scripts/lint-appimage-scripts.sh` (bash -n, shellcheck -S warning, smoke payload extract, double-quoted `$ORIGIN` scan). Agents must run it when pack/smoke scripts change. Documented in `.github/copilot-instructions.md`.


## AppImage blank page on host (after green 31250829176)

Not networking. WebProcess SIGABRT from host epiphany web-process extension GVariant skew (`@a(ss)` vs `a(ssb)`). libephymain still pointed at `/usr/lib64/epiphany/web-process-extensions`. Also default WebKit used host bwrap. Hot-patch reloc + sandbox-off AppRun loaded example.com (session title Example Domain). Packer/AppRun/smoke updated; rebuild next.


## AppImage render fix rebuild green (31252342415)

After commit 2087615, AppImage workflow success. Local session title Example Domain on example.com without manual ELF hot-patch. Blank-page kink closed for the private validation image path. Keep sandbox-off + extension reloc in packer forever for this AppImage.
