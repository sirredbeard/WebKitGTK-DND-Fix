...
#   untagged leftovers. Borrowed from sirredbeard/azurelinux-desktop and
#   pointed at WebKitGTK-DND-Fix-builder by default.
# Usage:
#   ./scripts/prune-ghcr-container.sh [PACKAGE] [KEEP_TAGGED]
#   GHCR_PACKAGE=webkitgtk-dnd-fix-builder ./scripts/prune-ghcr-container.sh [KEEP]
# Examples:
#   ./scripts/prune-ghcr-container.sh webkitgtk-dnd-fix-builder 2
#   ./scripts/prune-ghcr-container.sh 2
# Needs: gh auth with packages read/delete (packages:write in CI).
# CI:    build-container.yml after each image push.
#
# Env:
#   GHCR_OWNER   (default: sirredbeard)
#   GHCR_PACKAGE (default: webkitgtk-dnd-fix-builder; overridden by PACKAGE arg)
#
# Keep policy (default KEEP=2): newest KEEP tagged versions. Untagged
# versions are always deleted. Moving tags like :fedora44 just retarget a
# version; prune is by version id and created_at, not by tag name.

set -euo pipefail

OWNER="${GHCR_OWNER:-sirredbeard}"
KEEP=2
PACKAGE_RAW="${GHCR_PACKAGE:-webkitgtk-dnd-fix-builder}"

# Args: optional PACKAGE (non-numeric), optional KEEP (digits)
for arg in "$@"; do
    if [[ "$arg" =~ ^[0-9]+$ ]]; then
        KEEP="$arg"
    else
...
if ! [[ "$KEEP" =~ ^[0-9]+$ ]] || ((KEEP < 1)); then
    ...
    echo "   or: GHCR_PACKAGE=name $0 [KEEP]" >&2
    exit 2
fi
