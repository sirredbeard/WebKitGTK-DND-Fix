...
# SYNC_MODE=full|ccache|prefix|flatpak|images|quick|buildsnap  (default quick)
# flatpak = in-SDK WebKit last-good tarballs only (not flatpak-builder-state).
set -u
...
SDK_TAG="${FLATPAK_SDK_TAG:-sdk50}"

mkdir -p "$(dirname "$LOG")" "$CACHE"
exec >>"$LOG" 2>&1 || true
echo "==== $(date -u +%Y-%m-%dT%H:%M:%SZ) peer-sync mode=${MODE} ===="

# Peer sync exists to make real jobs faster. It is not a reason to hold compute
# allocated. It used to run hourly on a timer, which meant the Azure VM had a
# standing excuse to stay awake. Run it from workflow jobs, or pass
# SYNC_ON_DEMAND=1 by hand.
if [[ -f /etc/webkit-dnd/BUDGET_STOP ]]; then
  echo "budget stop active; skipping peer sync"
  exit 0
fi
if [[ "${SYNC_ON_DEMAND:-0}" != "1" && -z "${GITHUB_RUN_ID:-}" ]]; then
  echo "no workflow run and SYNC_ON_DEMAND unset; skipping peer sync"
  exit 0
fi
...
# In-SDK WebKit last-good (org.gnome.Sdk//50). Not Fedora prefix. Not builder-state.
# Files: webkitgtk-flatpak-sdk50-last-good.tar.zst, tip-named *.tar.zst, sha stamps.
sync_flatpak_webkit_handoff() {
  local ip="$1"
  local dir="$CACHE/flatpak-webkit"
  local rdir="/var/cache/webkit-dnd/flatpak-webkit"
  local lg="webkitgtk-flatpak-${SDK_TAG}-last-good.tar.zst"
  local stamp="last-good-flatpak-webkit-sha"
  local local_sz remote_sz local_sha remote_sha winner="" local_tip remote_tip tip_base
  mkdir -p "$dir" "$CACHE/out"

  local_sz=$(stat -c%s "$dir/$lg" 2>/dev/null || echo 0)
  remote_sz=$(ssh "${SSH_OPTS[@]}" "${SSH_USER}@${ip}" \
    "stat -c%s $rdir/$lg 2>/dev/null || echo 0" 2>/dev/null || echo 0)
  local_sha=$(tr -cd '0-9a-f' <"$dir/$stamp" 2>/dev/null | head -c 40 || true)
  remote_sha=$(ssh "${SSH_OPTS[@]}" "${SSH_USER}@${ip}" \
    "tr -cd '0-9a-f' <$rdir/$stamp 2>/dev/null | head -c 40 || true" 2>/dev/null || true)
  ...
  echo "flatpak-webkit handoff winner=${winner} local=${local_sz}b/${local_sha:-?} remote=${remote_sz}b/${remote_sha:-?}"

  if [[ "${winner}" == remote && "${remote_sz}" -gt 0 ]]; then
    "${RSYNC[@]}" -e "$(rsync_e)" "${SSH_USER}@${ip}:$rdir/$lg" "$dir/" 2>/dev/null || true
    "${RSYNC[@]}" -e "$(rsync_e)" "${SSH_USER}@${ip}:$rdir/$stamp" "$dir/" 2>/dev/null || true
  fi
  if [[ -f "$dir/$lg" ]]; then
    ssh "${SSH_OPTS[@]}" "${SSH_USER}@${ip}" "mkdir -p $rdir" 2>/dev/null || true
    "${RSYNC[@]}" -e "$(rsync_e)" "$dir/$lg" "${SSH_USER}@${ip}:$rdir/" 2>/dev/null || true
  fi
  if [[ -f "$dir/$stamp" ]]; then
    "${RSYNC[@]}" -e "$(rsync_e)" "$dir/$stamp" "${SSH_USER}@${ip}:$rdir/" 2>/dev/null || true
    "${RSYNC[@]}" -e "$(rsync_e)" "$dir/$stamp" \
      "${SSH_USER}@${ip}:/var/cache/webkit-dnd/out/" 2>/dev/null || true
    cp -f "$dir/$stamp" "$CACHE/out/last-good-flatpak-webkit-sha" 2>/dev/null || true
  fi

  # Tip-named tarballs (sha in filename). Pull newest remote + push newest local.
  local_tip=$(ls -1t "$dir"/webkitgtk-flatpak-${SDK_TAG}-*.tar.zst 2>/dev/null \
    | grep -v last-good | head -1 || true)
  remote_tip=$(ssh "${SSH_OPTS[@]}" "${SSH_USER}@${ip}" \
    "ls -1t $rdir/webkitgtk-flatpak-${SDK_TAG}-*.tar.zst 2>/dev/null | grep -v last-good | head -1" \
    2>/dev/null || true)
  if [[ -n "$remote_tip" ]]; then
    tip_base=$(basename "$remote_tip")
    if [[ ! -f "$dir/$tip_base" ]]; then
      echo "flatpak-webkit pull tip $tip_base"
      "${RSYNC[@]}" -e "$(rsync_e)" "${SSH_USER}@${ip}:$remote_tip" "$dir/" 2>/dev/null || true
    fi
  fi
  if [[ -n "$local_tip" ]]; then
    tip_base=$(basename "$local_tip")
    echo "flatpak-webkit push tip $tip_base"
    ssh "${SSH_OPTS[@]}" "${SSH_USER}@${ip}" "mkdir -p $rdir" 2>/dev/null || true
    "${RSYNC[@]}" -e "$(rsync_e)" "$local_tip" "${SSH_USER}@${ip}:$rdir/" 2>/dev/null || true
  fi
}
...
for ip in $PEERS; do
  ...
  case "$MODE" in
    ...
    flatpak) sync_flatpak_webkit_handoff "$ip" ;;
    images) sync_newest_image "$ip"; sync_dir "$ip" images ;;
    buildsnap) mkdir -p "$CACHE/build-snapshots"; sync_dir "$ip" build-snapshots ;;
    quick)  sync_dir "$ip" ccache; sync_prefix_handoff "$ip"; sync_flatpak_webkit_handoff "$ip"
            sync_newest_image "$ip"
