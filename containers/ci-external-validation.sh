...
if [[ -f "$GTEST_LOG" ]]; then
  ...
  # gtest lines like: [  PASSED  ] SelectionData.Foo (0 ms)
  while IFS= read -r line; do
    if [[ "$line" =~ PASSED[^A-Za-z0-9_]*([A-Za-z0-9_]+\.[A-Za-z0-9_]+) ]]; then
      TEST_STATUS["${BASH_REMATCH[1]}"]=PASS
    elif [[ "$line" =~ FAILED[^A-Za-z0-9_]*([A-Za-z0-9_]+\.[A-Za-z0-9_]+) ]]; then
...
# summary line from gtest
if [[ -f "$GTEST_LOG" ]]; then
  if grep -qE '\[  PASSED  \].*tests?\.|PASSED.*tests' "$GTEST_LOG"; then
    pass "gtest summary has PASSED"
  fi
  if grep -qE '\[  FAILED  \]' "$GTEST_LOG"; then
    bad "gtest log contains FAILED"
  fi
fi
