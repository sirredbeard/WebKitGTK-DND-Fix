...
# Which build configuration produced this log. gtk4/gtk3 build the GTK UIProcess
# drop target, wpe only shares the WebCore glib layer.
...
note "WEBKIT_CONFIG=$WEBKIT_CONFIG"
...
# SelectionData covers the WebCore glib trust split and is built by every port.
REQUIRED_TESTS=(
  ...
  SelectionData.PortalFilenamesNotWidenedByHostileURIList
  SelectionData.UriListWithoutFilenamesKeepsHttpOnly
  SelectionData.TrustedDropShapeAfterIpcRoundTrip
  SelectionData.TrustedDropPrefersPortalFilenames
  SelectionData.TrustedDropFallsBackToURIListWhenNoPortalList
  SelectionData.TrustedDropWithoutFilesGrantsNothing
  SelectionData.TrustedDropWithOnlyPortalListHasNoURIList
  SelectionData.TrustedDropReplacesPreviousFilenames
)

# DropTargetState is the GTK UIProcess drop lifecycle; PlatformWPE.cmake does not
# build it, so only require it from the GTK lanes.
if [[ "$WEBKIT_CONFIG" == gtk* ]]; then
  REQUIRED_TESTS+=(
    DropTargetState.SynchronousDropIsNotDeferred
    DropTargetState.DropDuringLoadIsDeferredAndReplayedOnce
    DropTargetState.DropIsNotDeferredWhenNothingIsLoading
    DropTargetState.LeaveAfterDeferredDropStillOwesFinish
    DropTargetState.LeaveWithoutDropOwesNothing
    DropTargetState.FinishedDropIsNotFinishedAgain
    DropTargetState.DestroyWithDeferredDropOwesFinish
    DropTargetState.AcceptResetsPreviousState
    DropTargetState.LoadingCompletionWithoutDropDoesNotDefer
  )
fi
...
# --- Auto layer checklist ---
{
  ...
  echo "fail=$fail"
} | tee "$OUT_DIR/layer-checklist-auto.txt" || true

# also keep classic checklist text (OUT may be root-owned from docker; best-effort)
if [[ -x "$ROOT/print-layer-checklist.sh" ]]; then
  "$ROOT/print-layer-checklist.sh" >"$OUT_DIR/layer-checklist.txt" 2>&1 \
    || "$ROOT/print-layer-checklist.sh" >"/tmp/layer-checklist.txt" 2>&1 \
    || true
fi
