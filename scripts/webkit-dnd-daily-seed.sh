...
if [[ -f /etc/webkit-dnd/BUDGET_STOP ]]; then
  echo "budget stop active; skipping maintenance"
  exit 0
fi
...
# Deliberately no activity stamp here. Maintenance is not project work, and
# stamping it is what let housekeeping hold the VM allocated indefinitely.
# The MAINTENANCE_LOCK above holds the watchdog off, and that lock expires.
mkdir -p "$CACHE/out"
