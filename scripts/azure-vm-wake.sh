...
WAKE_BLOCK_PCT="${WAKE_BLOCK_PCT:-100}"

# Compute is only allowed to start for real work on this project. A wake needs
# a workflow run behind it. Anonymous or scheduled wakes are what kept this VM
# allocated for three weeks in August 2026.
WAKE_REASON="${WAKE_REASON:-${GITHUB_WORKFLOW:-}}"
if [[ -z "$WAKE_REASON" ]]; then
  echo "refusing to wake: set WAKE_REASON, or run this from a workflow" >&2
  echo "compute starts for project work only" >&2
  exit 2
fi
echo "wake reason: ${WAKE_REASON}${GITHUB_RUN_ID:+ (run ${GITHUB_RUN_ID})}"
...
# Budget gate. Refuse to start compute when the month is already spent. The VM
# cannot enforce this itself while deallocated, so the check lives here too.
COST_MTD=""
if [[ -x "$(dirname "$0")/azure-cost-mtd.sh" ]]; then
  COST_MTD=$("$(dirname "$0")/azure-cost-mtd.sh" 2>/dev/null || true)
fi
if [[ "$COST_MTD" =~ ^[0-9.]+$ ]]; then
  pct=$(awk -v s="$COST_MTD" -v b="$BUDGET_USD" 'BEGIN { if (b <= 0) print 0; else printf "%.1f", (s / b) * 100 }')
  echo "month-to-date spend=${COST_MTD} USD budget=${BUDGET_USD} USD consumed=${pct}%"
  if awk -v p="$pct" -v t="$WAKE_BLOCK_PCT" 'BEGIN { exit !(p >= t) }'; then
    echo "BUDGET_BLOCKED: ${pct}% of the ${BUDGET_USD} USD budget is spent" >&2
    echo "refusing to start ${VM}; raise BUDGET_USD or wait for the month to roll" >&2
    exit 3
  fi
else
  echo "warning: month-to-date cost unreadable; relying on the on-VM guard" >&2
fi
